...
async function getAllMessages() {
  const { rows } = await pool.query(
    "SELECT u.first_name, u.last_name, m.id AS message_id, m.title, m.text, m.date, m.user_id FROM users u JOIN messages m ON u.id = m.user_id ORDER BY m.date DESC"
  );
  return rows;
}
...
async function getMessagesByUser(userId) {
  const { rows } = await pool.query(
    "SELECT u.first_name, u.last_name, m.id AS message_id, m.title, m.text, m.date, m.user_id FROM users u JOIN messages m ON u.id = m.user_id WHERE m.user_id = $1 ORDER BY m.date DESC",
    [userId]
  );
  return rows;
}
